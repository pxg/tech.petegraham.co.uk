			...
			switch ($type) {
				case 'category':
					...
					$items = $wpdb->get_results("
						...
						ON p.ID = pc.post_id
						WHERE pc.category_id = ".$custom['cat_ID']."
				...
				case 'category_popularity':
					...
					$items = $wpdb->get_results("
						SELECT DISTINCT c.cat_name AS cat_name, AVG(pop.total) AS avg
						FROM $wpdb->posts p
						LEFT JOIN $wpdb->ak_popularity pop
						ON p.ID = pop.post_id
						LEFT JOIN $wpdb->post2cat pc
						ON p.ID = pc.post_id
						LEFT JOIN $wpdb->categories c
						ON pc.category_id = c.cat_ID
						GROUP BY c.cat_ID
						ORDER BY avg DESC
					");
					$list = '';
